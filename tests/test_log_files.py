...
def test_01a_unknown_file_format_raises():
    ...
    assert exception.value.args[0] == "The format of the following log file(s) is unexpected: " \
                                      "'{}'.".format(log_files[0])
...
def test01b_malformed_log_files_raises():
    ...
    tests: R-LOG-FILES-001
    tests: R-LOG-FILES-003"""
    log_files = list(map(lambda file: join(dirname(__file__), TEST_DATA_DIR, file),
...
def test_01c_non_existent_log_file_raises():
    ...
    assert exception.value.args[0] == "The following log file(s) were not found: " \
                                      "'{}'.".format(log_files[0])
...
def test_02b_order_multiple_log_file_succeeds():
    ...
    tests: R-LOG-FILES-001
    tests: R-LOG-FILES-004"""
    log_files_ordered = [join(dirname(__file__), TEST_DATA_DIR, "good.1.log"),
                         join(dirname(__file__), TEST_DATA_DIR, "good.2.log")]
    log_files_unordered = [join(dirname(__file__), TEST_DATA_DIR, "good.2.log"),
