...
def test_01a_unknown_file_format_raises():
    ...
    log_files = [(Path(__file__).parent / TEST_DATA_DIR / "garbage").absolute()]
...
def test01b_malformed_log_files_raises():
    ...
    tests: R-LOG-FILES-003"""
    log_files = [
        Path(__file__).parent / TEST_DATA_DIR / file
    ...
    assert exception.value.args[0].index(str(log_files[0]))
    assert exception.value.args[0].index(str(log_files[1]))
    assert exception.value.args[0].index(str(log_files[2]))
    assert exception.value.args[0].index(str(log_files[3]))
...
def test_01c_non_existent_log_file_raises():
    ...
    tests: R-LOG-FILES-001
    tests: R-LOG-FILES-003"""
    log_files = [Path(__file__).parent / TEST_DATA_DIR / "non_existing.log"]
    with pytest.raises(RuntimeError) as exception:
        LogFiles(log_files)
...
def test_02a_order_single_log_file_succeeds():
    ...
    log_files = [Path(__file__).parent / TEST_DATA_DIR / "good.1.log"]
...
def test_02b_order_multiple_log_file_succeeds():
    ...
    tests: R-LOG-FILES-004"""
    log_files_ordered = [
        Path(__file__).parent / TEST_DATA_DIR / "good.1.log",
        Path(__file__).parent / TEST_DATA_DIR / "good.2.log",
    ]
    log_files_unordered = [
        Path(__file__).parent / TEST_DATA_DIR / "good.2.log",
        Path(__file__).parent / TEST_DATA_DIR / "good.1.log",
    ...
    log_files_ordered_compressed = [x.with_suffix(".log.gz") for x in log_files_ordered]
    lf = LogFiles(log_files_ordered_compressed)
    assert lf.file_names == log_files_ordered_compressed
    lf = LogFiles(x.with_suffix(".log.gz") for x in log_files_unordered)
    assert lf.file_names == log_files_ordered_compressed

    log_files_ordered_compressed = [x.with_suffix(".log.bz2") for x in log_files_ordered]
    lf = LogFiles(log_files_ordered_compressed)
    assert lf.file_names == log_files_ordered_compressed
    lf = LogFiles(x.with_suffix(".log.bz2") for x in log_files_unordered)
    assert lf.file_names == log_files_ordered_compressed

    log_files_ordered_compressed = [x.with_suffix(".log.lzma") for x in log_files_ordered]
    lf = LogFiles(log_files_ordered_compressed)
    assert lf.file_names == log_files_ordered_compressed
    lf = LogFiles(x.with_suffix(".log.lzma") for x in log_files_unordered)
    assert lf.file_names == log_files_ordered_compressed
